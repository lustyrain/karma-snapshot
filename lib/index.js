 ...
 * @param {string} browserName Name of the client browser.
 * @returns Full path to snapshot file.
 */
function defaultPathResolver(basePath, suiteName, browserName) {
  const suiteSourcePath = path.join(basePath, browserName, suiteName);
  const suiteSourceDir = path.dirname(suiteSourcePath);
  const sourceFileName = path.basename(suiteName);

  return path.join(basePath, "__snapshots__", browserName, sourceFileName + ".md");
}
...
function snapshotFramework(files, config, emitter, loggerFactory) {
  ...
  emitter.on('browser_complete', (clientInfo, data) => {
    ...
    const browserName = clientInfo.name;
        ...
        if (!lastResult.error && lastResult.failed === 0 && lastResult.skipped === 0) {
          ...
          if (prunedSnapshots.pruned.length > 0) {
            if (snapshotConfig.prune) {
              ...
              prune.pruneFiles(snapshotConfig.pathResolver, config.basePath, prunedSnapshots.prunedFiles, browserName);
              dirty = true;
            } else {
        ...
        if (dirty) {
          Object.keys(rootSuite.children).forEach((suiteName) => {
            ...
            if (suite.dirty) {
              ...
              const snapshotPath = snapshotConfig.pathResolver(config.basePath, suiteName, browserName);
